...
class AppStore extends Component{
    ...
    getFilteredApps = ( getSearchResult) =>{
        const {activeTabId} = this.state
        const filteredApps =   getSearchResult.filter((eachApp)=>(
    ...
    render(){
        ...
        return(
            <div className="app-store-bg">
                ...
                <input type="search" className="search-bar" placeholder="Search" onChange={this.onChangeSearchInput}
                    value={searchInput}
                /> 
                </div>
                <ul className="tabs-list">
                    {tabsList.map((eachTodo)=>(
                        <TabItem key={eachTodo.tabId} tabItemDetails={eachTodo} updateApps={this.updateApps}
                            isActive={eachTodo.tabId === isActiveTab}
                        />
                    ))}
                </ul>
                <div className="all-apps-container">
                    <ul className="apps-list">
                        {filteredAppValues.map((eachItem)=>(
                            <AppItem appItemDetails={eachItem} key={eachItem.appId}/>
                        ))}
                    </ul>
                </div>
            </div>
